    ...
    <div className="relative z-20">
      {confirmOptions.map((option, index) => {
        ...
        return (
          <button
            key={index}
            className={`btn absolute -translate-x-1/2 -translate-y-1/2 transform rounded-full bg-[#3c6e71] p-3 text-white transition duration-300 ${isOpen ? "opacity-100" : "opacity-0"}`}
            style={{
